...
import type { ActiveKeys, CanvasName, KeyName, SimActions, SimBundle, SimRefs, Snapshot, Tab, Theme } from '../types'

// Simulation 인스턴스를 만들고 rAF 루프 생명주기를 관리하며,
// 상태 스냅샷·시계·키보드(WASD)·캔버스 ref·액션을 React에 노출한다.
export default function useSimulation(): SimBundle {
  const simRef = useRef<Simulation | null>(null)
  if (simRef.current === null) simRef.current = new Simulation()
  const sim = simRef.current

  const [status, setStatus] = useState<Snapshot>(() => sim.snapshot())
  const [clock, setClock] = useState('--:--:--')
  const [activeTab, setActiveTabState] = useState<Tab>('robot') // 순찰 로봇 관제가 첫 페이지
  const [activeKeys, setActiveKeys] = useState<ActiveKeys>({ w: false, a: false, s: false, d: false })
  const [theme, setTheme] = useState<Theme>('dark') // 'dark' | 'light' — 두 페이지 공통

  // 최신 탭 값을 키보드 핸들러에서 참조 (핸들러는 1회만 등록)
  const tabRef = useRef<Tab>(activeTab)
  ...
  useEffect(() => {
    const cctvMap: Record<KeyName, string> = { w: 'u', a: 'l', s: 'd', d: 'r' }
    const arrowMap: Record<string, KeyName> = { arrowup: 'w', arrowdown: 's', arrowleft: 'a', arrowright: 'd' }
    const resolve = (e: KeyboardEvent): KeyName | null => {
      let key = e.key.toLowerCase()
      if (arrowMap[key]) { key = arrowMap[key]; e.preventDefault() } // 방향키 → WASD, 페이지 스크롤 방지
      return 'wasd'.includes(key) ? (key as KeyName) : null
    }
  ...
  const canvasRef = useCallback((name: CanvasName) => (el: HTMLCanvasElement | null) => { if (el) sim.registerCanvas(name, el) }, [sim])
  const refs = useMemo<SimRefs>(() => ({
    cam3: canvasRef('cam3'),
