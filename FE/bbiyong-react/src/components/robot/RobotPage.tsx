...
export default function RobotPage() {
  ...
  // 🔴 [2026-08-13] CameraPage 와 같은 이유로 videoSeen.FRONT 를 쓰지 않는다 — 영상이
  //    WebSocket 을 떠나 그 값이 영원히 false 다. WebRTC(WHEP) 재생 상태로 판정한다.
  //    videoRef 는 검출 박스 오버레이가 표시 영역·해상도를 읽는 데 공유한다.
      ...
      <div className="b-grid">
        ...
        <div className="panel" id="pCam">
          ...
          <div className={`vwrap${camDown ? ' down' : ''}`}>
            {/* 라이브는 WebRTC <video>(WHEP), 시뮬레이션은 종전 캔버스(Simulation.drawRcam 이 그린다) */}
            {enabled ? (
              <>
                <video ref={frontVideoRef} autoPlay playsInline muted style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                {/* 박스는 WS 로 즉시 온다. WebRTC 영상은 ~0.3초 지연이라 그만큼만 늦춰 맞춘다(WEBRTC_LAG_S). */}
