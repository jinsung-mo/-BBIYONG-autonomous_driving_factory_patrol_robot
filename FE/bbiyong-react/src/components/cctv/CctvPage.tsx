...
import DateLogModal from './DateLogModal'
import LogList from '../LogList'
import type { Detection } from '../../types'

// 화재 감지 카드 (불꽃/연기 상세)
function DetectCard({ det, fireOn }: { det: Detection; fireOn: boolean }) {
  return (
...
export default function CctvPage({ active }: { active: boolean }) {
  const { status, refs } = useSim()
  const { det, fireOn } = status
  const [sound, setSound] = useState(true)     // 경보음 토글 (표시 전용)
  const [pickedDay, setPickedDay] = useState<number | null>(null) // 클릭한 달력 날짜
