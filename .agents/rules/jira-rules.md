...
### 6.1 완료 기준(DoD) 필수·구체화 규칙
* **모든 이슈는 "무엇이 끝나면 완료인지"를 최소 1줄 이상** 완료 기준에 구체적으로 적습니다. 완료 기준이 비어 있거나 제목만 반복하는 티켓은 발행하지 않습니다.
* 모호한 표현(예: "구현 완료", "작업함") 대신 **검증 가능한 조건**으로 작성합니다.
  * 예시: `문서 초안 작성 완료`, `기능 구현 후 화면에서 동작 확인`, `발표 시연 가능한 상태 확인`, `API 200 응답 및 단위테스트 통과`
* AI가 티켓을 자동 생성할 때도 이 규칙을 적용하여 완료 기준을 반드시 채웁니다.

## 7. 스프린트 관리 규칙 (Sprint Hygiene)
스프린트가 끝나면 **반드시 스프린트 종료 처리**를 하고, 다음 주 작업은 **새 스프린트를 만들어 시작(Start)** 까지 눌러 활성화합니다. 끝난 스프린트 안에 `해야 할 일 / 진행 중 / 완료` 이슈가 섞여 남지 않도록 관리합니다.

* **스프린트 종료 시 정리 절차**:
  1. 완료된 이슈는 `완료(Done)` 상태로 정리
  2. 끝내지 못한 이슈는 **다음 스프린트 또는 백로그로 이동**
  3. 더 이상 하지 않는 작업은 **백로그에서 정리**(보류/삭제)
  4. 종료 후 스프린트에 미완료 이슈가 남아있지 않은지 확인
* **새 스프린트 시작**: 다음 주 작업을 새 스프린트에 담고 **`스프린트 시작(Start sprint)`** 을 눌러 활성화합니다.
* **수행 위치**: 스프린트 종료/시작은 Jira 보드(Backlog 화면)에서 수행합니다. (기본 이슈 REST API 대상이 아니며, 필요 시 Agile API `/rest/agile/1.0/sprint`로 보조)
