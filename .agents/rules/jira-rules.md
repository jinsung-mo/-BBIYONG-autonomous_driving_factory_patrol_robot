...
## 8. 티켓 우선 발급 및 개발 자동화 규칙 (Ticket-First Workflow)
* **티켓 우선 발급 원칙 (No Ticket, No Work)**: AI 에이전트는 사용자가 코딩, 기능 구현, 문서 작성 등의 새로운 개발 태스크를 요청하면, **실제 파일 수정이나 코드 작성을 시작하기 전에 반드시 지라 티켓부터 생성**해야 합니다.
* **자동화 실행 순서**:
  1. **티켓 설계**: 사용자 요청 분석 후 Epic/Story 매핑 및 컨벤션에 맞는 제목과 본문 템플릿(Jira용 h3., * [ ] 사용)을 작성합니다.
  2. **지라 등록**: Jira REST API를 호출하여 티켓을 먼저 발행합니다.
  3. **티켓 안내**: 발급된 지라 티켓 키(예: `S15P11E101-224`)를 사용자에게 먼저 보고합니다.
  4. **로컬 브랜치 생성**: 발급된 티켓 키를 포함하여 기능 개발 브랜치(`feat/S15P11E101-224-branch-name`)를 로컬에서 자동으로 생성하고 전환합니다.
  5. **개발 진행**: 이후 코딩 작업을 진행하고 커밋 및 MR 생성 단계로 진입합니다.
* AI 에이전트는 이 티켓 우선 프로세스를 사람의 개입 없이 자동으로 순차 처리해야 합니다.
