...
| 실제 장치 주행 | 관제 웹 연동 | AI 처리량 개선 | 관제 영상 지연 |
| :--- | :--- | :--- | :--- |
| 실내 장애물 환경에서 이동 시연 | 지도·영상·제어·이벤트 이력 연결 | **33.37 → 70.60 FPS · 2.12배** | **약 6초 → 0.3초 안팎** |

AI 수치는 같은 장치와 FP16 조건에서 측정한 파이프라인 결과입니다. [측정 조건](AI/deployment/fire_smoke/README.md#verified-jetson-result-2026-08-03) 영상 지연은 HLS 재생 구조(2초 세그먼트)와 WebRTC 전환 후 관제 화면의 보정값을 기준으로 적었습니다.
...
| 항목 | 내용 |
| --- | --- |
| 관제 백엔드 | 관제 백엔드 에픽 담당. 에픽 이슈 132건 중 94건 처리 |
| 코드 | 커밋 249개(팀 667개 중 37%). `BE_system/src` 코드의 약 83% 작성 |
| 테스트 | 테스트 코드 작성·수정 커밋 72개 (서버 테스트 전체 약 210건) |
| 통합 | 파트별 dev → main 승격과 최종 릴리스 통합 담당 |

<sub>커밋과 이슈 수는 팀이 사용한 원본 GitLab 저장소와 Jira 기준입니다. 이 저장소는 결과물을 옮겨 온 사본이라 커밋 이력이 하나로 합쳐져 있습니다.</sub>

**주요 작업**

- **로봇·서버·웹 실시간 통신**: 로봇 전용 WebSocket(`/ws/robot`)과 관제 웹용 STOMP 채널을 나눠 설계했습니다. 처음에는 TCP 9000번 포트로 구현했는데 서버 방화벽이 443을 제외한 포트를 막고 있는 것을 확인했습니다. 그래서 Nginx를 거치는 WSS(443)로 바꿨고 메시지 형식은 그대로 두어 다른 파트는 코드를 고칠 필요가 없었습니다.
...
경보를 DB에 저장하는 작업이 WebSocket 수신 스레드에서 바로 실행되고 있었습니다. 저장이 늦어지면 텔레메트리와 영상 수신까지 함께 밀렸습니다. 경보가 들어온 순서와 중복 판정 순서가 바뀌면 안 되므로 저장 작업을 비동기로 옮기되 스레드는 하나만 두었습니다. 대기열(200건)이 차면 호출한 쪽이 직접 처리하므로 경보를 버리지 않고 속도만 늦춥니다.
...
SQLite에서 MySQL로 옮긴 이유도 여기에 있습니다. SQLite용 Hibernate 방언이 FK와 UNIQUE 제약을 오류 없이 무시해서 중복을 막을 마지막 장치가 없었습니다. 전환 과정은 바로 아래 항목에 정리했습니다.
...
SQLite는 쓰기 잠금을 DB 파일 전체에 겁니다. 여러 커넥션이 동시에 쓰면 `database is locked` 오류가 나기 때문에 처음부터 커넥션 풀을 1로 묶어 두었습니다. 잠금 오류는 사라졌지만 모든 조회와 쓰기가 커넥션 하나를 기다리게 됐습니다. 경보 저장이 WebSocket 수신 스레드에서 실행되던 구조와 겹쳐, 저장이 밀리면 텔레메트리와 영상 수신까지 같이 멈췄습니다. 오류를 없앤 대신 대기가 애플리케이션 스레드로 옮겨 온 셈입니다.
...
영상 경로는 팀과 함께 서버 밖으로 옮겼고, 저는 관제 화면을 WebRTC로 바꾸는 작업을 맡았습니다. 먼저 HLS로 바꾸면서 로봇 송신량이 12.3Mbps에서 8.2Mbps로 줄었지만 지연이 6초쯤 됐습니다. 화재 알림은 바로 오는데 영상은 6초 뒤에 보이는 상태였고, 이 지연으로는 원격 조종을 할 수 없었습니다. 최종적으로 로봇이 H.264로 한 번 인코딩해 MediaMTX로 올리고 브라우저가 WebRTC(WHEP)로 받는 구조로 바꿔 지연을 0.3초 안팎으로 줄였습니다.

영상이 빠진 뒤에는 STOMP 송신 버퍼도 4MB에서 512KB로 줄였습니다. 남은 데이터가 초당 수십 KB뿐이라 큰 버퍼는 몇 초 전 지도와 온도를 쌓아 두기만 했기 때문입니다. 영상이 STOMP를 떠나기 전에 줄이면 오히려 연결이 더 자주 끊기므로 로봇 쪽 전환 로그를 확인한 다음에 줄였습니다.
...
- **AI 도구 규칙 통일**: 팀원마다 Claude Code, Gemini, Codex, Cursor 등 다른 도구를 썼습니다. 한 팀원이 AI로 다른 파트 코드를 한 번에 만들어 1만 줄이 넘는 MR이 올라온 뒤로, 같은 규칙을 도구별 설정 파일(`CLAUDE.md`, `GEMINI.md`, `CODEX.md`, `.cursorrules`)에 똑같이 넣었습니다.
