...
| 테스트 | 서버 테스트 클래스 42개, 실행 기준 약 210건 |
...
- **경보를 한 번만, 순서대로 기록**: 로봇이 같은 경보를 다시 보내도 한 건만 남도록 멱등 키와 DB 유니크 제약을 함께 걸었습니다. 경보 저장은 단일 스레드 비동기 처리로 옮겨 순서를 지키면서도 실시간 수신은 멈추지 않습니다.
...
SQLite에서 MySQL로 옮긴 이유도 여기에 있습니다. SQLite용 Hibernate 방언이 FK와 UNIQUE 제약을 오류 없이 무시해서 중복을 막을 마지막 장치가 없었습니다.
...
팀과 함께 영상 경로를 서버 밖으로 옮겼습니다. 먼저 HLS로 바꾸면서 로봇 송신량이 12.3Mbps에서 8.2Mbps로 줄었지만 지연이 6초쯤 됐습니다. 화재 알림은 바로 오는데 영상은 6초 뒤에 보이는 상태였고, 이 지연으로는 원격 조종을 할 수 없었습니다. 최종적으로 로봇이 H.264로 한 번 인코딩해 MediaMTX로 올리고 브라우저가 WebRTC(WHEP)로 받는 구조로 바꿔 지연을 0.3초 안팎으로 줄였습니다.

영상이 빠진 뒤에는 STOMP 송신 버퍼를 4MB에서 512KB로 줄였습니다. 남은 데이터가 초당 수십 KB뿐이라 큰 버퍼는 몇 초 전 지도와 온도를 쌓아 두기만 했기 때문입니다. 영상이 STOMP를 떠나기 전에 줄이면 오히려 연결이 더 자주 끊기므로 로봇 쪽 전환 로그를 확인한 다음에 줄였습니다.
